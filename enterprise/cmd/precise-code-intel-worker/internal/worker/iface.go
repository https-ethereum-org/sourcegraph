...
type DBStore interface {
	...
	GetDumpForCommit(ctx context.Context, repositoryID int, commit, indexer, root string) (dbstore.Dump, bool, error)
}
...
type LSIFStore interface {
	...
	DocumentsReferencing(ctx context.Context, bundleID int, paths []string) ([]string, error)

	ReadMeta(ctx context.Context, bundleID int) (lsifstore.MetaData, error)
	PathsWithPrefix(ctx context.Context, bundleID int, prefix string) ([]string, error)

	ReadDocument(ctx context.Context, bundleID int, path string) (lsifstore.DocumentData, bool, error)
	ReadResultChunk(ctx context.Context, bundleID int, id int) (lsifstore.ResultChunkData, bool, error)
	ReadDefinitions(ctx context.Context, bundleID int, scheme, identifier string, skip, take int) ([]lsifstore.LocationData, int, error)

	ReadReferences(ctx context.Context, bundleID int, scheme, identifier string, skip, take int) ([]lsifstore.LocationData, int, error)
...
type GitserverClient interface {
	...
	DiffFileStatus(ctx context.Context, repositoryID int, baseCommit, headCommit string) (map[string]gitserver.Status, error)
}
