...
type UploadHandler struct {
	...
	gitserverClient codeintelgitserver.Client
	internal        bool
}

func NewUploadHandler(lsifStore LSIFStore, dbStore DBStore, uploadStore uploadstore.Store, gitserverClient codeintelgitserver.Client, internal bool) http.Handler {
...
func (h *UploadHandler) handlePathsToIndex(r *http.Request, uploadArgs UploadArgs) (interface{}, error) {
	...
	dumps, err := h.dbStore.FindClosestDumps(ctx, uploadArgs.RepositoryID, uploadArgs.Commit, uploadArgs.Root, false, uploadArgs.Indexer)
	if err != nil {
		return nil, err
	}
	...
	statuses, err := h.gitserverClient.DiffFileStatus(ctx, uploadArgs.RepositoryID, baseCommit, uploadArgs.Commit)
	...
	pathsToIndex, err := h.lsifStore.DocumentsReferencing(ctx, dump.ID, diffedPaths)
